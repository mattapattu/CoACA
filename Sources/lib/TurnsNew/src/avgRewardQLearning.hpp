...
std::vector<double> getQLearningLik(Rcpp::S4 ratdata, Rcpp::S4 modelData, Rcpp::S4 testModel, int sim, bool debug)
{
  arma::mat allpaths = Rcpp::as<arma::mat>(ratdata.slot("allpaths"));
  std::string model = Rcpp::as<std::string>(modelData.slot("Model"));
  arma::mat turnTimes;
  ...
  for (unsigned int session = 0; session < uniqSessIdx.n_elem; session++)
  {
    ...
    while(i < (nrow))
    {
      ...
      for (int j = 0; j < nbOfTurns; j++)
      {
        ...
        //Rcpp::Rcout <<"Determined siblings of curNode"  << std::endl;
        ...
        {
          //Rcpp::Rcout <<"Curr action is not greedy" << std::endl;
        }
        ...
        //If curr turn is an intermediate turn in the maze, determine qmax using edges
        if(edges->size() > 0) 
        {
        ...
        //If curr turn leads to next box, then select qmax using actions from next box
