...
//namespace sarsa{
Rcpp::List simulateSarsa2(Rcpp::S4 ratdata, Rcpp::S4 modelData, Rcpp::S4 testModel, arma::vec turnStages, bool debug)
{
...
std::vector<double> getSarsaLik(Rcpp::S4 ratdata, Rcpp::S4 modelData, Rcpp::S4 testModel, int sim)
{
  arma::mat allpaths = Rcpp::as<arma::mat>(ratdata.slot("allpaths"));
  std::string model = Rcpp::as<std::string>(modelData.slot("Model"));
  arma::mat turnTimes;
  ...
  for (unsigned int session = 0; session < uniqSessIdx.n_elem; session++)
  {
    ...
    //Rcpp::Rcout <<"states_sess="<<states_sess<<std::endl;
    //Rcpp::Rcout <<"actions_sess="<<actions_sess<<std::endl;
    ...
    while (i < (nrow - 1))
    {
      ...
      while (!endOfCurrEpisode && i < (nrow - 1))
      {
        ...
        {
          endOfCurrEpisode = true;
        }
        
        //Rcpp::Rcout <<"S=" <<S << ", R="<< R << ", nbOfTurns=" << nbOfTurns << ", endOfCurrEpisode=" << endOfCurrEpisode << std::endl;
        ...
        for (int j = 0; j < nbOfTurns; j++)
        {
          ...
          {
            double prediction = gamma * currNode->credit;
            double td_err = prevTurnReward + prediction - prevNode->credit;
            prevNode->credit = prevNode->credit + (alpha * td_err);
            //Rcpp::Rcout <<"prevNode="<< prevNode->node << ", prevTurnReward=" << prevTurnReward << ", td_err=" <<td_err << ", prevNode.credit=" << prevNode->credit <<std::endl;
            
          }
          if (j == (nbOfTurns - 1) && endOfCurrEpisode)
          {
            double td_err = currTurnReward - currNode->credit;
            currNode->credit = currNode->credit + (alpha * td_err);
            //Rcpp::Rcout <<"currNode="<< currNode->node << ", currTurnReward=" << currTurnReward << ", td_err=" <<td_err << ", currNode.credit=" << currNode->credit <<std::endl;
          }
          prevNode = currNode;
...
arma::mat getSarsaProbMat(Rcpp::S4 ratdata, Rcpp::S4 modelData, Rcpp::S4 testModel, int sim)
{
  arma::mat allpaths = Rcpp::as<arma::mat>(ratdata.slot("allpaths"));
  std::string model = Rcpp::as<std::string>(modelData.slot("Model"));
  arma::mat turnTimes;
      ...
      while (!endOfCurrEpisode && i < (nrow - 1))
      {
        ...
        for (int j = 0; j < nbOfTurns; j++)
        {
          ...
          if(j==0)
          {
            ...
            edge = graph->getEdge(rootNode->node, currNode->node);
            //Rcpp::Rcout <<"src=" <<rootNode->node << ", dest =" <<  currNode->node  <<std::endl;
          }
          else
          {
            edge = graph->getEdge(prevNode->node, currNode->node);
            //Rcpp::Rcout <<"src=" <<prevNode->node << ", dest =" <<  currNode->node <<std::endl;
          }
